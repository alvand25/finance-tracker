...
def process_image(image_path: str, target_dir: str = TARGET_DIR) -> Dict[str, str]:
    """
    ...
    Returns:
        Dictionary with paths to the processed files
    """
    # Get base filename without extension
    base_name = os.path.basename(image_path).split('.')[0]
    
    # Define output paths
    target_image_path = os.path.join(IMAGE_DIR, os.path.basename(image_path))
    ocr_path = os.path.join(OCR_DIR, f"{base_name}.txt")
    
    # Copy the image to the target directory
    if not os.path.samefile(image_path, target_image_path):
    ...
    save_text_to_file(extracted_text, ocr_path)
...
def main():
    ...
    if args.process_all or args.vendor:
        for image_path in image_paths:
            ...
            files = process_image(image_path, TARGET_DIR)
            
            # Create annotation
            if args.annotate:
                annotation = interactive_annotation(files["ocr"], files["image"])
            else:
                annotation = create_annotation_template(files["ocr"], files["image"])
            
            # Save annotation
            annotation_path = save_annotation(annotation, files["image"])
