"""Trader Joe's receipt handler implementation."""
...
class TraderJoesReceiptHandler(BaseReceiptHandler):
    """Handler for Trader Joe's receipts."""
    ...
    def can_handle_receipt(self, text: str) -> float:
        ...
        # Check for typical Trader Joe's formatting
        format_confidence = 0.0
        if re.search(r'TRADER\s*JOE[\'S]*\s*#\d+', text):
            format_confidence += 0.4
        if re.search(r'THANK\s*YOU.*TRADER\s*JOE', text, re.IGNORECASE):
    ...
    def extract_store_info(self, text: str) -> Tuple[str, float]:
        ...
        store_match = re.search(r'TRADER\s*JOE[\'S]*\s*#(\d+)', text)
        if store_match:
            self.store_number = store_match.group(1)
            store_name = f"TRADER JOE'S #{self.store_number}"
            confidence = 1.0
        else:
        ...
        for pattern in self.TOTAL_PATTERNS:
            ...
            if match:
                try:
                    ...
                    if 0 < total < 10000:  # Reasonable range for Trader Joe's
                        return total, 0.9
                    else:
